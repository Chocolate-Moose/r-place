...
contract RedirectAll is SuperAppBase {
   ...
   constructor(
        ...
        // create mappings to store grid and gridColors
        mapping (uint256 => bool) grid;
        mapping (uint256 => string) gridColors;
    ...
    // TODO: check type internal
    // check if incoming pixels are available to start stream
    // function isValid(
    //     bytes calldata _ctx
    // )
    //     internal   
    ...
    //     //  TODO: Vico: given ctx, decode CTX and check if (gridIndex, colorCode) gridIndex is 0 or 1. 
    //     //     If 0: set to 1 and set colorcode and start stream
    //     //     Else: return false
    //     uint i;
    //     uint arrayLength = selectedPixels.length;
    //     for (i = 0; i < arrayLength; i++) {
    //         if (grid[selectedPixels[i]] == true) {  // if pixel already has a stream
    //             return false;
    //         }
    //     }
    //     return true;
